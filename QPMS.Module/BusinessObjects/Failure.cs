...
namespace QPMS.Module.BusinessObjects
{
    ...
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
           ...
           && string.IsNullOrEmpty(Code))
            {
                ...
                Code = string.Format("FAILURE-{0:D8}", nextSequence);
            }
        ...
        PTF pTF;
        int quantity;
        MediaDataObject failPhoto;
        Project project;
        string description;
        string code;
        string name;

        [RuleRequiredField("RuleRequiredField for Failure.Name", DefaultContexts.Save, "A name must be specified")]
        ...
        [RuleRequiredField("RuleRequiredField for Failure.Project", DefaultContexts.Save, "A project must be specified")]
        [Association("Project-Failures")]
        ...
        public MediaDataObject FailPhoto
        {
            get => failPhoto;
            set => SetPropertyValue(nameof(FailPhoto), ref failPhoto, value);
        }

        [DataSourceProperty("Project.Parts")]
        [Association("Part-Failures")]
        public XPCollection<Part> PartNumbers
        {
            get
            {
                return GetCollection<Part>(nameof(PartNumbers));
            }
        }

        [DataSourceProperty("Project.FailureModes")]
